...
@ActiveProfiles("feignInWebflux")
@SpringBootTest(classes = AutoConfigTestApplication.class)
class FeignInWebfluxConfigTests {

	@Test
	void test(ApplicationContext applicationContext) {
		assertNotNull(applicationContext.getBean(ServletRequestIdHandler.class));
		assertNotNull(applicationContext.getBean(RequestIdInterceptor.class));
		assertNotNull(applicationContext.getBean(RequestThreadContextIdHandler.class));
	}
