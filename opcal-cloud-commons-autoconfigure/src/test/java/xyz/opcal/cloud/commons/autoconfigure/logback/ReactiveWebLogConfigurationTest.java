...
class ReactiveWebLogConfigurationTest {
	...
	static ApplicationContextRunner contextRunner() {
		return new ApplicationContextRunner().withConfiguration(
				AutoConfigurations.of(ReactiveWebLogConfiguration.class, JacksonAutoConfiguration.class, RefreshAutoConfiguration.class));
	}

	@Test
	void enableLogRequest() {
		contextRunner().withPropertyValues("opcal.cloud.log.reactive.request=true").run(context -> {
	...
	void enableLogRequestId() {
		contextRunner().withPropertyValues("opcal.cloud.log.reactive.request-id=true").run(context -> {
			assertThat(context).hasSingleBean(ReactiveRequestIdFilter.class);
			assertThat(context).hasSingleBean(LogWebfluxRequestIdFilter.class);
			assertThat(context).hasSingleBean(MDCCleanFilter.class);
